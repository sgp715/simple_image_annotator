...
def writeLabelsKITTI(image):
    with open(app.config["OUT"]+re.sub('\.(jpg|jpeg|png|tif)','.txt',image),'w') as f:
        for label in app.config["LABELS"]:
...
def tagger():
    ...
    if not image == app.config["CURFIL"]:
        ...
        current_file = app.config["OUT"]+re.sub('\.(jpg|jpeg|png|tif)','.txt',app.config["CURFIL"])
...
def next():
    image = app.config["FILES"][app.config["HEAD"]]
    if app.config["FORMAT"] == "KITTI":
        writeLabelsKITTI(image)
    else:
        writeLabelsCSV(image)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-k", "--kitti", help='specify that you want the output format to be KITTI', action="store_true")
    ...
    app.config["HEAD"] = 0
    if args.kitti != None:
        app.config["FORMAT"] = "KITTI"
        if args.out == None:
            app.config["OUT"] = args.dir
        else:
            app.config["OUT"] = args.out
        if not app.config["OUT"].endswith("/"):
            app.config["OUT"] += "/"
    else:
        app.config["FORMAT"] = "CSV"
        if args.out == None:
